...
folder_path = r'C:\mygit\SkyRoot\raw_data'

# 결과 저장 폴더
output_folder = r'C:\mygit\SkyRoot\compressed_data'  
...
for file_name in tqdm(os.listdir(folder_path), desc="Processing files"):
    if file_name.endswith('.csv'):
        ...
        # 압축된 결과를 저장할 빈 데이터프레임 생성
        compressed_results = pd.DataFrame()

        # TRIP_NO 그룹 진행 상황 표시
        trip_no_groups = df['TRIP_NO'].nunique()
        with tqdm(total=trip_no_groups, desc=f"Processing {file_name}") as pbar:
            # 각 TRIP_NO 그룹에 대해 처리
            for trip_no, group in df.groupby('TRIP_NO'):
                ...
                # 원본 컬럼 순서 유지
                compressed_group = compressed_group[df.columns]
                ...
                compressed_results = pd.concat([compressed_results, final_group], ignore_index=True)
        ...
        # 결과를 파일 이름에 '_compressed' 추가하여 저장
        output_file = os.path.join(output_folder, f"{os.path.splitext(file_name)[0]}.csv")
        compressed_results.to_csv(output_file, index=False)

print("모든 원본 데이터의 압축이 완료되었습니다.")